...
class BigPlayerGUI(tk.Frame, BaseGUIEvent.BaseGUIEvent):
    try:
        def __init__(self, parent, controller):
            ...
            self.var = IntVar()
            AscBtn = Radiobutton(self, text='Ascending', variable=self.var, value=2, command=self.sortList)
            AscBtn.pack(anchor=W)
            DesBtn = Radiobutton(self, text='Descending', variable=self.var, value=1, command=self.sortList)
        ...
        def load(self, *args, **kwargs):
            ...
            if "someF225" in self.controller.globals:
                return
            self.controller.globals["someF225"] = True

            # Calling of global file path procurement.csv
            path2 = self.controller.globals["procurement_file_path"]

            # Fetch descending order of big player data from PBFunction.py
            data = PBFunction.bigPlayerFunc(path2)
